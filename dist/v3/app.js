...
(()=>{
 ...
 const selection=()=>({system:Number($('assembly-system')?.value||0),size:$('sauna-size').value,storage:$('sauna-storage').checked,roof:Number($('sauna-roof').value),terrace:Number($('terrace-depth').value)/600,window:Number($('window-width').value),facade:Number($('facade').value)});
 ...
 async function load(){
  ...
  try{
   ...
   const download=document.createElement('p');const a=document.createElement('a');a.href='generated/OBTP_Grasshopper_R06.zip';a.textContent='Download this revision’s GH authoring scripts';download.append(a);$('program').append(download);
   ...
   show();window.OBTPUpdatePDF?.();window.OBTPSuppliersRender?.(scene);
