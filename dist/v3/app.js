...
(()=>{
 ...
 function show(){
  ...
  $('solved-plan').src=`solved-plans/sauna-${s.size}-${s.storage?'storage':'open'}.svg`;
  $('assembly-title').textContent=`Sauna ${s.size.toUpperCase()}${s.storage?' · external storage':''}${solved?' · owner reference plan':''}`;
  ...
  $('view-description').textContent=solved?'Original owner reference drawing; new finishes, window and terrace are shown in the 3D model.':view==='cut'?'Cut at 1.10 m above floor · fixed camera':'Fixed camera · Rotate 90° or scroll to zoom';
 ...
 async function load(){
  ...
  try{
   ...
   $('program').replaceChildren();for(const text of scene.holds){const p=document.createElement('p');p.textContent=text;$('program').append(p);}
   ...
   const download=document.createElement('p');const a=document.createElement('a');a.href='generated/OBTP_Grasshopper_Source.zip';a.textContent='Download this revision’s GH authoring scripts';download.append(a);$('program').append(download);
