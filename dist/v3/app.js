...
(()=>{
 ...
 function render(){
  try {
   ...
   $('program').replaceChildren();const heading=document.createElement('strong');heading.textContent=program.name+' · program allowances';const use=document.createElement('p');use.textContent=program.use;const list=document.createElement('ul');for(const name of program.zones){const li=document.createElement('li');li.textContent=name;list.append(li);}const caveat=document.createElement('small');caveat.textContent='Planning labels only: no interior partitions, access openings, services or wet-room assembly in the generated model.';$('program').append(heading,use,list,caveat);
