...
(()=>{
 ...
 function render(){
  try {
   ...
   const {scene,metrics,program,preset,researchHold}=presets.create(api,{preset:$('preset').value,bays,columns,rows,height,layer:$('layer').value});
   ...
   window.OBTPStudioV3={scene,items:scene.items,renderer,bays,columns,rows,preset,metrics,program,researchHold};
 ...
 $('preset').addEventListener('change',()=>{const matrix=$('preset').value==='matrix';$('matrix-fields').hidden=!matrix;$('bays-field').hidden=matrix;
