...
(()=>{
 ...
 function limitSauna(){
  ...
  $('sauna-circulation-note').textContent=chosen.bays+' cassette modules · hot room, little front hall and required outdoor shower'+(chosen.storage?'; separate 1.2 × 1.2 m side storage study outside the generated shell':'')+'. Exterior services, annex structure and openings remain proposals.';
 }
 ...
 function render(){
  try {
   ...
   currentStudy=sauna?linked.outdoorView(api,scene,plan):null;
   currentStudy?.models.forEach(m=>renderer.register(m));
   ...
   if(isMatrix){const detail=document.createElement('p');detail.textContent=(columns*rows)+' planning cells · '+columns+' columns × '+rows+' rows. Straight wall bays use the shared 600 mm cassette; floors and roofs are grouped up to 2,400 × 1,200 mm with separate perimeter pieces.';const caveat=document.createElement('small');caveat.textContent='Walls follow only the outside boundary. The corner posts and bearing details are placeholders; no room partitions, openings, supports or verified connections are generated.';$('program').append(detail,caveat);}
    ...
    diagram.style.gridTemplateColumns=`repeat(${plan.storage?9:plan.columns},1fr)`;diagram.style.gridTemplateRows=`repeat(${plan.rows},minmax(28px,1fr))`;
    ...
    for(const side of (plan.circulation==='compact'?['front']:({shared:['front'],sharedTwoAccess:['west','east'],wetLobby:['front'],deadEnd:['front'],through:['front','rear']})[plan.circulation])){const mark=document.createElement('span');mark.className='plan-door '+side;mark.setAttribute('aria-hidden','true');if(plan.circulation==='shared'||plan.circulation==='wetLobby')mark.style.left='43%';if(plan.circulation==='compact')mark.style.left='72%';diagram.append(mark);}
    ...
    const placement=document.createElement('p');placement.textContent='Generated sub-block fit: '+(plan.subblocks.status==='spatial-candidate'?'candidate · heater, benches, exterior shower'+(plan.storage?', unbuilt side storage study':'')+' and access edges placed':'no fit · '+plan.subblocks.failures.join('; '))+'. The plan view shows these reservations.';
    ...
    const reference=document.createElement('p'),link=document.createElement('a');link.href='https://github.com/anotherSipOfCoffee/obtp-studio/blob/main/docs/SAUNA_SIDE_STORAGE_R12.md';link.target='_blank';link.rel='noopener';link.textContent='Review compact Sauna and unresolved details ↗';reference.append(link);$('program').append(diagram,facts,placement,download,cadNote,caveat,reference);
