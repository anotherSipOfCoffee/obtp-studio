...
await page.locator('#customer-info').click();await v3.locator('#studio-version').selectOption('v2');await v2.locator('#status').filter({hasText:/component instances · ready/}).waitFor();assert(await v2.locator('#bays').isVisible());await v2.locator('#studio-version').selectOption('v3');await ready();await page.locator('#customer-config').click();
await page.setViewportSize({width:390,height:844});await page.locator('#customer-info').click();await v3.locator('[data-view="solved"]').click();await v3.locator('#solved-plan').evaluate(img=>img.decode());assert(await v3.locator('#solved-scroll').evaluate(x=>x.scrollWidth<=x.clientWidth && x.scrollHeight<=x.clientHeight));assert(await v3.locator('body').evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'studio-sauna-plan-mobile.png',fullPage:true});
await page.locator('#language').selectOption('lt');assert.equal(await v3.locator('html').getAttribute('lang'),'lt');assert.match(await v3.locator('h1').first().textContent(),/Susikurkite savo pirtį/);await page.locator('#customer-info').click();assert.match(await v3.locator('#customer-information h1').textContent(),/Techninė informacija/);await page.locator('#customer-config').click();assert.equal(await v3.locator('#window-width').inputValue(),'1200');await page.screenshot({path:'studio-customer-lt.png',fullPage:true});
