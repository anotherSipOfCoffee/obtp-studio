...
async function ready(){await v3.locator('#status').filter({hasText:/Your sauna preview|Jūsų pirties peržiūra/}).waitFor({timeout:60000});}
await ready();
...
for(const width of ['600','900','1200']){await choose('window-width',width);await ready();assert.equal(await v3.locator('canvas').evaluate(()=>OBTPStudioV3.scene.config.window_width),Number(width));}
await choose('terrace-depth','600');await ready();assert.equal(await v3.locator('canvas').evaluate(()=>OBTPStudioV3.scene.config.terrace_steps),1);
...
await page.locator('#customer-drawings').click();assert(await v3.locator('#customer-drawings').isVisible());const pdfURL=await v3.locator('#pdf-download').getAttribute('href');const pdfResponse=await page.request.get(new URL(pdfURL,new URL('v3/',page.url())).href);assert(pdfResponse.ok());assert((await pdfResponse.body()).subarray(0,4).toString()==='%PDF');await page.screenshot({path:'studio-r04-drawings.png',fullPage:true});await page.locator('#customer-config').click();assert.equal(await v3.locator('canvas').evaluate(()=>OBTPStudioV3.renderer.arctic),true);await v3.locator('#appearance').selectOption('materials');assert.equal(await v3.locator('canvas').evaluate(()=>OBTPStudioV3.renderer.arctic),false);await v3.locator('#appearance').selectOption('arctic');await page.locator('#customer-product').click();assert.equal(await v3.locator('.photo-empty').count(),3);await page.screenshot({path:'studio-r04-gallery.png',fullPage:true});await page.locator('#customer-config').click();await page.screenshot({path:'studio-r04-customer.png',fullPage:true});await choose('sauna-roof','2');await ready();assert.equal(await v3.locator('canvas').evaluate(()=>OBTPStudioV3.metrics.height_mm),4480);await page.screenshot({path:'studio-r04-gable.png',fullPage:true});assert.deepEqual(errors,[]);console.log('PASS: R04 exported-model defaults, all buyer controls, source identity roundtrip, cut, locked camera, wood, reference plans, mobile and WikiHouse version');
